...
#### Problem Lösung Mostafa: mehr Übertragung von einer Location zu den beiden anderen als 
...
def opt_dc(nr_time_steps, nr_cooling_machines=4, nr_fwp=2, nr_locations=3, nr_pipes=3, cop=4, LOAD_STEPS_PER_HOUR=4, SOC_start=0, eta_laden=0.95, eta_entladen=0.97, SOC_max=28660, SOC_ende=0):
   ...
   # model.PIPES = en.Set(initialize=np.arange(nr_pipes))
    ...
    model.import_level = en.Var(model.LOCA, model.LOCA, model.T, domain=en.NonNegativeReals)
    ...
    def cover_load_rule(model, loca, t):  
        return sum(model.km_generation_power[loca, i, t] for i in model.KM) + sum(model.import_level[locai,loca,t] for locai in model.LOCA) >= model.cooling_load[loca, t] + model.speicher_laden[t] - model.speicher_entladen[t]
    ...
    def import_level_rule(model, locai, locaj, t):
        return model.import_level[locai, locaj, t] <= model.surplus[locai,t] * model.transfer_losses[(locai, locaj)] 
    model.import_lvl = en.Constraint(model.LOCA, model.LOCA, model.T, rule=import_level_rule)
